...
export default function CausePieChart({ data }: { data: RootCauseStats }) {
  ...
  if (!rows.length) {
    return <p className="py-10 text-center text-sm text-slate-400">No classified root causes yet.</p>;
  }
  return (
    <div className="flex flex-wrap items-center gap-4">
      <div className="h-52 w-52 shrink-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={rows} dataKey="count" nameKey="label" innerRadius={44} outerRadius={78} paddingAngle={2} stroke="#fff">
      ...
      <ul className="min-w-0 flex-1 space-y-1">
        {rows.map((d, i) => (
          <li key={d.label} className="flex items-center gap-2 text-xs text-slate-600">
            ...
            <span className="ml-auto shrink-0 font-medium tabular-nums text-slate-500">{d.pct}%</span>
          </li>
        ))}
      </ul>
    </div>
