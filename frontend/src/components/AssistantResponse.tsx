  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {a.service && <Badge tone="bg-brand-50 text-brand-600 ring-1 ring-brand-100">{a.service}</Badge>}
        {a.error && <Badge tone="bg-slate-100 text-slate-700 ring-1 ring-slate-200">{a.error}</Badge>}
        ...
        <span className="ml-auto flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
          ...
          <Link to={`/incidents/${ai.incident_id}`} className="font-medium text-brand-600 hover:underline">Full record →</Link>
        </span>
      </div>

      {/* summary */}
      <p className="text-sm leading-relaxed text-slate-700">{ai.summary || a.keywords?.join(", ")}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="card p-4">
            <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Probable Root Causes</h4>
                ...
                <li key={`${c.label}-${i}`} className="flex gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-800">
                      ...
                      <span className="text-xs font-normal text-slate-500">probability: {c.probability}</span>
                    </p>
                    <p className="mt-0.5 text-[13px] leading-relaxed text-slate-600">{c.description}</p>
                    {!!c.evidence.length && (
                      <p className="mt-1 text-[11px] text-slate-500">
                        Evidence: {c.evidence.map((e) => <span key={e} className="mr-1 font-mono text-brand-600">{e}</span>)}
                      </p>
                    )}
                  </div>
                </li>
            ...
            {ai.troubleshooting.length ? (
              <ol className="mt-2 space-y-2">
                {ai.troubleshooting.map((s, i) => (
                  <li key={i} className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-slate-800">{s.step}</p>
                      <p className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px]">
                        {s.evidence.map((ev) => (
                          <span key={ev} className="rounded bg-white px-1.5 py-0.5 font-mono text-slate-500 ring-1 ring-slate-200">{ev}</span>
                        ))}
          ...
          <div className="card p-4">
            <div className="flex items-center justify-between">
              <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Supporting Evidence</h4>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4">
            <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Similar Incidents</h4>
            {ai.similar_incidents.length ? (
              <ul className="mt-2 space-y-2">
                {ai.similar_incidents.map((s) => (
                  <li key={s.public_id} className="rounded-lg border border-slate-100 bg-slate-50/70 px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="ml-auto text-xs font-semibold tabular-nums text-emerald-700">{pct(s.similarity)}</span>
                    </div>
                    ...
                    <p className="mt-1.5 line-clamp-2 text-xs text-slate-600">{s.title}</p>
                    {s.root_cause && <p className="mt-1 text-[11px] text-slate-500">cause: {s.root_cause}</p>}
          ...
          <div className="card p-4">
            <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Retrieval Detail</h4>
            ...
            {!!ai.retrieval.queries.length && (
              <details className="mt-2">
                <summary className="cursor-pointer text-[11px] font-medium text-slate-500">queries used</summary>
                <ol className="mt-1 space-y-1">
                  {ai.retrieval.queries.map((q, i) => (
                    <li key={i} className="rounded bg-slate-50 px-2 py-1 font-mono text-[10px] leading-relaxed text-slate-600">
                      {i + 1}. {q}
                    </li>
                  ))}
                </ol>
