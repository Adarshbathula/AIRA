    ...
    <Modal open={open} onClose={busy ? () => undefined : onClose} title="Ingest enterprise documents" width="max-w-xl">
      <div
        className={`flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 py-8 text-center transition-colors
          ${dragOver ? "border-brand-500 bg-brand-50" : "border-slate-300 dark:border-neutral-700 bg-slate-50/60 dark:bg-white/5"}`}
      ...
      >
        <div className="text-3xl text-slate-400 dark:text-slate-500">⇪</div>
        <p className="text-sm text-slate-600 dark:text-slate-300">Drag & drop, or</p>
        ...
        />
        <p className="text-[11px] text-slate-400 dark:text-slate-500">PDF · DOCX · DOC · TXT · MD · HTML · CSV · XLSX · JSON · LOG — max 15 MB each</p>
      </div>

      {files.length > 0 && (
        <ul className="mt-3 space-y-1">
          {files.map((f, i) => (
            <li key={`${f.name}-${i}`} className="flex items-center gap-2 rounded-md border border-slate-100 dark:border-white/5 bg-white dark:bg-[#262626] px-2.5 py-1.5 text-xs">
              <span className="truncate font-medium text-slate-700 dark:text-slate-300">{f.name}</span>
              <span className="text-slate-400 dark:text-slate-500">{(f.size / 1024).toFixed(0)} KB</span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <label className="block">
          <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">Category (optional — auto-detected if blank)</span>
        ...
        <label className="block">
          <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">Service</span>
          <input className="input" placeholder="Payment API" value={meta.service} onChange={(e) => setMeta({ ...meta, service: e.target.value })} />
        </label>
        <label className="block">
          <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">Department</span>
          <input className="input" placeholder="SRE" value={meta.department} onChange={(e) => setMeta({ ...meta, department: e.target.value })} />
        </label>
        <label className="block">
          <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">Author / version</span>
      ...
      {progress && <p className="mt-3 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400"><Spinner className="h-3.5 w-3.5" /> {progress}</p>}

      <div className="mt-4 flex items-center justify-between">
        <p className="text-[11px] text-slate-400 dark:text-slate-500">Pipeline: detect → parse → metadata → clean → chunk → embed → FAISS upsert</p>
