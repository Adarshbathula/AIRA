...
export default function AppLayout() {
  ...
  return (
    <div className="flex min-h-screen">
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-ink-950 transition-transform lg:static lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-16 items-center gap-2.5 border-b border-white/10 px-4">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500">Self-Healing RAG · LangGraph</p>
          </div>
        </div>

        <nav className="space-y-0.5 px-2 py-4">
          <p className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400">Workspace</p>
          {NAV.map((n) => (
          ...
          {isAdmin && (
            <>
              <p className="px-3 pb-1 pt-4 text-[10px] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400">Administration</p>
              {ADMIN_NAV.map((n) => (
                <NavLink key={n.to} to={n.to} className={({ isActive }) => `sidebar-link ${isActive ? "sidebar-link-active" : ""}`}>
                  <span className="text-base leading-none">{n.icon}</span> {n.label}
                </NavLink>
        ...
        <div className="mx-3 mt-2 rounded-lg bg-ink-900 p-3 text-[11px] leading-relaxed text-slate-400 dark:text-slate-500">
      ...
      {open && <div className="fixed inset-0 z-20 bg-slate-900/40 lg:hidden dark:bg-black/60" onClick={() => setOpen(false)} />}

      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex h-16 items-center gap-3 border-b border-slate-200 dark:border-white/10 bg-white/90 dark:bg-[#171717]/90 px-4 backdrop-blur sm:px-6">
          <button className="rounded-md border border-slate-200 dark:border-white/10 px-2.5 py-1.5 text-slate-600 dark:text-slate-300 lg:hidden" onClick={() => setOpen((o) => !o)} aria-label="Menu">
            ☰
          </button>
          <h1 className="truncate text-sm font-semibold text-slate-800 dark:text-slate-200">
        ...
        <main className="min-w-0 flex-1 p-4 sm:p-6">
            {offlineNotice && (
              <div className="mb-4 flex items-start gap-3 rounded-lg border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 px-3 py-2 text-sm text-amber-800 dark:text-amber-300">
                <span aria-hidden>⚠</span>
                <p className="flex-1">{offlineNotice}</p>
                <button
                  onClick={retry}
                  disabled={retrying}
                  className="rounded-md border border-amber-300 bg-white dark:bg-[#262626] px-2 py-0.5 text-xs font-medium text-amber-800 dark:text-amber-300 hover:bg-amber-100 hover:dark:bg-amber-900/40 disabled:opacity-60"
                >
